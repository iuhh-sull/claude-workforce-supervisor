...
| 入口 | 适用场景 | 成本边界 |
|------|----------|----------|
| `run` | 一次性调研、提取、文本或数字判断 | `-p` 同步执行；必须显式给 `-MaxTurns` 和 `-MaxBudgetUsd`；默认保存 session，错误后可续接 |
| `start` | 需要后台运行、长期保留并可随时接回的员工 | 官方 `--bg` 不支持硬预算；靠阶段化派单、状态监控和 stop 止损 |
| `reply` | 给既有员工追加短增量任务 | 通过 `-p --resume`；必须显式给 `-MaxTurns` 和 `-MaxBudgetUsd`，并返回 subtype/cost/usage |
...
第三方定价映射不适用时，`maxBudgetUsd` 仍按 Claude Code 的内置美元估值触发，只能视为“Claude 等价执行上限”，不能按供应商人民币预算直接换算。按已观测的内置费率为最终回答留余量，并同时用 `maxTurns`、工具/读取范围、输出上限和供应商控制台约束真实费用；不要因假高估值在搜证完成后截掉交付。

- 需要处理约 10k tokens 以上原始材料，CC 可以只返回 1k-3k tokens 的证据摘要。
- 涉及六个以上文件的检索、分类、机械审查或一致性修改。
- 需要在单个巨大、压缩或生成文件中定向搜索，或要完成三个以上彼此独立的只读检索，Codex 只需接收证据索引。
...
- 三个以内普通文件、约 5k tokens 以内相关上下文、短文精修或几处明确的小补丁；巨大/压缩/生成文件不按文件数豁免。
...
需要硬上限的一次性任务使用 `run` 或 Claude Code MCP，并显式设置有限的 `MaxTurns`、`MaxBudgetUsd` 和返回大小。官方后台 `--bg` 不支持 `--max-budget-usd`，因此 `start` 不得声称或模拟硬预算；后台员工要拆成可验收阶段并按状态 stop。到达预算约 80%、连续两个实质回合重复读取或重复结论、开始修改范围外文件、擅自删减要求，或预计返工成本已超过 Codex 直接完成时，停止会话并由 Codex 接管；不要因为已有投入而继续追加预算。
...
```powershell
pwsh -NoProfile -File "<skill-dir>/scripts/claude-workforce.ps1" -Action run -Mode inspect -NoTools -Ephemeral -ContextProfile minimal -MaxTurns 1 -MaxBudgetUsd 1 -Model "deepseek-v4-flash[1m]" -Effort low -Cwd "<project>" -Prompt "只回复结构化判断"

pwsh -NoProfile -File "<skill-dir>/scripts/claude-workforce.ps1" -Action run -Mode inspect -ContextProfile project -MaxTurns 4 -MaxBudgetUsd 2 -Model "deepseek-v4-flash[1m]" -Effort medium -Cwd "<project>" -Prompt "搜索公开资料并返回带来源的简短结论"
...
```powershell
pwsh -NoProfile -File "<skill-dir>/scripts/claude-workforce.ps1" -Action reply -Id "<worker-id>" -Mode inspect -ContextProfile project -MaxTurns 2 -MaxBudgetUsd 2 -Prompt "基于前面的结论补充一项检查"
```

`reply` 未指定 `-ContextProfile` 时也按 `auto` 处理：加 `-NoTools` 选 `minimal`，否则选 `project`，默认不继承 MCP。纯文本连通性或记忆测试加 `-NoTools -Effort low`。`reply` 是同步的，适合短增量任务；需要持续后台监控的长任务使用 `respawn` 后手动 attach，或派发新的后台员工。
