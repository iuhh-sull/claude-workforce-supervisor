...
`new-workforce-session-profile.ps1` 在员工启动时生成一份临时配置，MCP 和 Agent View 共用。这份配置只活在当前员工会话里，不碰 `~/.claude/settings.json`。你自己跑 `claude` 时用的模型和权限完全不受影响。

Read、Glob、Grep、WebSearch、Plan 默认放行。但如果路径指向 `.env`、私钥、认证文件、`~/.codex/config.toml` 或 `~/.claude/settings.json`，更高优先级的 ask 规则会弹出来确认。员工只在当前任务明确授权后才能读用户配置，就算读到密钥也不能回显或外发。

Bash、Edit、Write、NotebookEdit、每个 WebFetch 目标、Agent 以及带副作用的 MCP 工具都会进入权限中间层。公开 URL 和明确只读的 shell/Git 检查，Codex 可以直接批；写入、往外发数据、认证、安装、发布、部署这些按每次的具体输入来定。

MCP 调用不在顶层预填 `allowedTools` 或 `disallowedTools`。社区维护的 `claude-code-mcp` 用 `canUseTool` 把待审批的请求交给 `claude_code_check`，Codex 再通过 `respond_permission` 逐条处理。批准只管当前这一次，不会存成 Claude 的长期权限。

Agent 默认走 ask。传了 `-AllowNestedAgents` 才会在当前员工会话里改为 allow；只在任务确实能并行、范围清楚、成本算得过来的时候开。

旧参数 `AllowBroadWebFetch` 还能读，兼容已有私有配置，但它不再帮你跳过 URL 审核。Claude Code 的 `auto` 模式这里也没用上：官方要求 Anthropic API 加受支持的 Claude 模型，当前 DeepSeek provider 不满足。

`maxTurns` 当请求边界看就好，别当硬保证。实测中 SDK/provider 曾超过设置值。真正靠得住的止损信号是重复读取、范围漂移、实际进度和供应商费用。
